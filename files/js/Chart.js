...
scaleHop = Math.floor(scaleHeight/calculatedScale.steps)-1;
calculateXAxisSize();
...
{ ctx.lineWidth = config.barStrokeWidth; 
	for (var i = 0; i < data.datasets.length; i++) { 
		for (var x = 0; x < data.datasets[i].data.length; x++){
			totalVotos=totalVotos+data.datasets[i].data[x];
		}
		for (var j = 0; j < data.datasets[i].data.length; j++) { 
 ...
 ctx.fillStyle = data.datasets[i].fillColor; 
 ctx.strokeStyle = data.datasets[i].strokeColor; 
 ctx.beginPath(); ctx.moveTo(barOffset, xAxisPosY); 
 ...
 ctx.closePath(); //added code to show data labels in the bar chart //you can add more config properties to remove these hardcoded values 
 ctx.fill(); 
 ctx.fillStyle = '#0B0B61';
 ctx.font = '14pt '+config.scaleFontFamily; 
 porcentaje=(data.datasets[i].data[j]*100)/totalVotos;
 if(j==0){
 	posicionFill=barWidth/(1.5);
 }else{
 	posicionFill=barWidth/2;
 }
 ctx.fillText(porcentaje.toFixed()+"%", barOffset + (posicionFill), xAxisPosY - animPc * calculateOffset(data.datasets[i].data[j], calculatedScale, scaleHop) + (config.barStrokeWidth / 2) - 12); ctx.restore(); 
} 
} 
}
...
while((numberOfSteps*stepValue)<=maxValue){
	numberOfSteps++;
}
while((numberOfSteps*stepValue)>(maxValue+stepValue)){
	numberOfSteps--;
}
